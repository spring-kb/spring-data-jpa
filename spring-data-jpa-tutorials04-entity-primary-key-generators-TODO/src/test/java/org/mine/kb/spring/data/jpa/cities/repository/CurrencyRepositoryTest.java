...
import org.mine.kb.spring.data.jpa.cities.specification.CurrencySpecification;
...
class CurrencyRepositoryTest {
	...
	@Test
	void testFindAllWithCurrencySpecification() {
		Currency probe = new Currency();
		probe.setCode("USD");
		CurrencySpecification spec = new CurrencySpecification(probe);
		List<Currency> result = currencyRepository.findAll(spec);
		assertThat(result).hasSize(1);
		assertThat(result.get(0).getCode()).isEqualTo("USD");
		assertThat(result.get(0).getDescription()).isEqualTo("Dollar");
	}
